...
func (s *State) GenNewTable() *Table {
	...
	for attempts := 0; attempts < 10; attempts++ {
		...
		if collationType != CollationGBKChineseCI && collationType != CollationGBKBin {
			newTbl.Collate = Collations[collationType]
			break
		}
	}
	// Fallback to utf8mb4 if all attempts failed
	if newTbl.Collate == nil {
		newTbl.Collate = Collations[CollationUtf8mb4GeneralCI]
	}
...
func (c *Column) RandomValuesAsc(count int) []string {
	...
	case ColumnTypeChar, ColumnTypeVarchar, ColumnTypeBinary, ColumnTypeVarBinary:
		...
		// Only use Chinese characters for GBK charset, use ASCII-only for other charsets
		// Default to false (ASCII-only) if Collation is nil to ensure compatibility
		mixCNChar := false
		if c.Collation != nil && c.Collation.CharsetName == "gbk" {
			mixCNChar = true
		}
		return RandStrings(length, count, mixCNChar)
...
var asciiRunes = []rune("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")

func RandStringRunes(n int, mixCNChar bool) string {
	b := make([]rune, n)
	for i := range b {
		// Only generate Chinese characters if explicitly requested (for GBK charset)
		// Otherwise, always use ASCII-only characters to ensure compatibility
		if mixCNChar && rand.Intn(3) == 0 {
			// Generate Chinese character for GBK charset only
			b[i] = rune(int('\u4e00') + rand.Intn(int('\u9fff')-int('\u4e00')))
		} else {
			// Use ASCII-only characters to ensure compatibility with all charsets
			// This prevents encoding errors for non-UTF8 charsets like latin1
...
func RandStrings(strLen int, count int, mixCNChar bool) []string {
	...
	for i := 0; i < count; i++ {
		// Ensure we generate at least 1 character
		actualLen := rand.Intn(strLen)
		if actualLen == 0 {
			actualLen = 1
		}
		// Generate string with proper charset handling
		str := RandStringRunes(actualLen, mixCNChar)
...
func RandGoTimes(count int, format string) []string {
	// For TIMESTAMP and DATETIME, MySQL/TiDB valid range starts at 1970-01-01 00:00:01 UTC.
	// Use 1970-01-02 to avoid boundary issues like '1970-01-01'.
	min := time.Date(1970, 1, 2, 0, 0, 1, 0, time.UTC).Unix()
