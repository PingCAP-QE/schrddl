...
func (col *ddlTestColumn) randValue() any {
	...
	case KindVector:
		...
		for i := 0; i < cnt; i++ {
			...
			str += fmt.Sprintf("%f", rand.Float64())
		}
		str += "]"
		return str
	default:
...
func (col *ddlTestColumn) canHaveDefaultValue() bool {
	switch col.k {
	case KindBLOB, KindTINYBLOB, KindMEDIUMBLOB, KindLONGBLOB, KindTEXT, KindTINYTEXT, KindMEDIUMTEXT, KindLONGTEXT, KindJSON, KindVector:
		return false
	default:
		return true
	}
...
func kToType(k int) sqlgenerator.ColumnType {
	...
	case KindVector:
		return sqlgenerator.ColumnTypeVector
	default:
