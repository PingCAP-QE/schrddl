...
func testImpoMutateCommon(t *testing.T, sql string, seed int64) {
	addr := os.Getenv("IMPOMYSQL_ADDR")
	if addr == "" {
		t.Skip("set IMPOMYSQL_ADDR to run impomysql tests")
	}

	fmt.Println("==================================================")
	// prepare table COMPANY
	if err := testsqls.InitTableCOMPANY(addr); err != nil {
		t.Fatalf("%+v", err)
	}
	// prepare connector
	conn, err := testsqls.GetConnector(addr)
	if err != nil {
		t.Fatalf("%+v", err)
	}
